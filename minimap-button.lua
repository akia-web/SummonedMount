...
local SUMMONEDMOUNT_LDB = LibStub("LibDataBroker-1.1"):NewDataObject("SUMMONEDMOUNT!", {
    ...
    OnClick = function(_, button)
        ...
        if button == "LeftButton" then
            Settings.OpenToCategory("SummonedMount");
        end
    end,
...
function SUMMONEDMOUNT_MM:OnInitialize()
    self.db = LibStub("AceDB-3.0"):New("SUMMONEDMOUNTMMDB", { profile = { minimap = { hide = false, }, }, })
    icon:Register("SUMMONEDMOUNT!", SUMMONEDMOUNT_LDB, self.db.profile.minimap)
end